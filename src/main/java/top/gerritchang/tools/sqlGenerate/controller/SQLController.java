package top.gerritchang.tools.sqlGenerate.controller;
...
import top.gerritchang.tools.sqlGenerate.service.GenerateSQLService;
    ...
    private Map generateOracleInsertSQL(String table_name, String table_schema, String primaryKey, String uniKey) {
        ...
        if ("".equals(uniKey)) {
            for (Map map : list) {
                if (!primaryKey.equals(map.get("COLUMN_NAME").toString())) {
                    String column_name = map.get("COLUMN_NAME").toString();
                    columnBuffer.append("<if test=\"").append(column_name).append(" != null");
